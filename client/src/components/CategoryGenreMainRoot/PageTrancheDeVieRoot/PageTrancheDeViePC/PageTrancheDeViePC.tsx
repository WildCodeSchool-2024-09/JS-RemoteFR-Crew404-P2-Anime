...
function PageTrancheDeViePC() {
  ...
  const DataAnimeTrancheDeViePCAPI = () => {
    fetch(`https://api.jikan.moe/v4/anime?genres=36&page=${count}`)
      .then((response) => response.json())
      .then((data) => {
        if (data?.data) {
          const nouvelAnime = data.data;
          setDataAnimeTrancheDeVie((prevData) => [...prevData, ...nouvelAnime]); // Concatène correctement
        } else {
  ...
  const ButtonAnimeTrancheDeViePC = (nombreAppels = 1) => {
    for (let i = 0; i < nombreAppels; i++) {
      DataAnimeTrancheDeViePCAPI();
    }
  };
...
export default PageTrancheDeViePC;
