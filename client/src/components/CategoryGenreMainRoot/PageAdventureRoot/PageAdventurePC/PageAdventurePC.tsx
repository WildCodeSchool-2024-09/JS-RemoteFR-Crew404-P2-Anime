...
function PageAdventurePC() {
  ...
  const DataAnimeAdventurePCAPI = () => {
    fetch(`https://api.jikan.moe/v4/anime?genres=2&page=${count}`)
      .then((response) => response.json())
      .then((data) => {
        if (data?.data) {
          const nouvelAnime = data.data;
          setDataAnimeAdventure((prevData) => [...prevData, ...nouvelAnime]); // Concatène correctement
        } else {
  ...
  const ButtonAnimeAdventurePC = (nombreAppels = 1) => {
    for (let i = 0; i < nombreAppels; i++) {
      DataAnimeAdventurePCAPI();
    }
  };
...
export default PageAdventurePC;
